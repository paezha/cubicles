...
I was inspired by this [beautiful creation](https://mastodon.online/@nrennie@fosstodon.org/109389979155033662) of [Nicola Rennie](https://mastodon.online/@nrennie@fosstodon.org) to try something similar using packages [{sf}](https://r-spatial.github.io/sf/) and [{rayshader}](https://www.rayshader.com/).

The color palettes are from Blake R. Mills [{MetBrewer}](https://github.com/BlakeRMills/MetBrewer) package, and my own [{MexBrewer}](https://paezha.github.io/MexBrewer/) collection.
...
```{r include=FALSE}
# Read named image
im <- imager::load.image(glue::glue(here::here(), "/outputs/hexicles-{seed_1}-{seed_2}.png"))
```

```{r include=FALSE}
im <- imager::imsub(im,x > 450 & x < 1700, y > 390 & y < 1640)
```

```{r include=FALSE}
imager::save.image(im, file = glue::glue(here::here(), "/outputs/hexicles-{seed_1}-{seed_2}.png"))
```

```{r echo=FALSE, out.width="500px"}
# Display image
knitr::include_graphics(glue("outputs/hexicles-{seed_1}-{seed_2}.png"))
```
